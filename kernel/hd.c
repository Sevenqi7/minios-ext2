...
static void partition(int device, int style)
{
	...
	if (style == P_PRIMARY) {
		...
		for (i = 0; i < NR_PART_PER_DRIVE; i++) { /* 0~3 */
			...
			//真的是醉了，ext2被我搭在逻辑分区里，这下面这一段根本就没机会跑，不知道为什么不注释掉一段就没法跑
			// else
			// {
			// 	get_fs_flags(drive, hdi->logical[dev_nr].base+2, &fs_flags_buf);
			// 	if(((struct ext2_superblock*)&fs_flags_buf)->s_magic == 0XEF53)
			// 		hdi->logical[dev_nr].fs_type = EXT2_TYPE;
			// }
		...
		for (i = 0; i < NR_SUB_PER_PART; i++) {
			...
			{
				memset(&fs_flags_buf, 0, sizeof(struct fs_flags));
				get_fs_flags(drive, hdi->logical[dev_nr].base+2, &fs_flags_buf);		//EXT2 flags is located in the second sector
				if(((struct ext2_superblock*)&fs_flags_buf)->s_magic == 0XEF53)
					hdi->logical[dev_nr].fs_type = EXT2_TYPE;
			}
			// else if(((struct ext2_superblock*)fs_flags_buf2)->s_magic == 0XEF53)
			// 	hdi->logical[dev_nr].fs_type = EXT2_TYPE;
