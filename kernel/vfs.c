...
void init_fileop_table()
{
    ...
    // table[3] for ext2
    f_op_table[3].create = ext2_create;
    f_op_table[3].open = ext2_open;
    f_op_table[3].close = ext2_close;
    f_op_table[3].write = ext2_write;
    f_op_table[3].lseek = ext2_lseek;
    f_op_table[3].unlink = ext2_unlink;
    f_op_table[3].read = ext2_read;
    f_op_table[3].createdir = ext2_mkdir;
    f_op_table[3].getdents = ext2_getdents;
}
...
void init_super_block_table(){
    ...
    //super_block[5] is ext2's superblock
    sb->sb_dev = DEV_HD;
    sb->fs_type = EXT2_TYPE;
    sb++;
...
static void init_vfs_table(){  // modified by mingxuan 2020-10-30
    ...
    //  ext2  Jiang Xiaotian 2023-1-8
    vfs_table[5].fs_name = "ext2";
...
static int get_index(char path[]){
    ...
    for(i=0;i<NR_FS;i++)    //modified by mingxuan 2020-10-29
    {
        ...
        {
            if(!strcmp(fs_name, "ext2"))
            {
                //JiangXiaotian 2023-1-23 写ext2的接口的时候路径全部默认带有根目录的'/'
                //但是这个函数会把路径的第一个'/'给去掉
                //出于省事的目的，就在这里加多了一个判断，把'/'给加回来
                int j;
                path[pathlen-a+1] = '\0';
                for(j=pathlen-a;j>0;j--)
                    path[j] = path[j-1];
                path[0] = '/';
            }
            return i;
...
int sys_getdents(void *uesp){
    return do_vgetdents(get_arg(uesp, 1), (struct linux_dirent *)get_arg(uesp, 2), get_arg(uesp, 3));
}
...
int do_vwrite(int fd, const char *buf, int count) {
    ...
    int f_len = count;
    int bytes = vfs_table[index].op->write(fd, buf, count);
    // while(f_len)
    // {
...
int do_vcreate(char *filepath) { //modified by mingxuan 2019-5-17
    ...
    fd = vfs_table[index].op->create(pathname);
    if(fd != -1)
        p_proc_current -> task.filp[fd] -> dev_index = index;
    /***
     * 这里有个fat32作者写的奇怪的状态机。create应该返回创建的文件的fd才对
     * 因此把这里注释掉
     * 2022-2-1 JiangXiaotian
    if (state == 1) {
...
int do_vgetdents(int fd, struct linux_dirent *dirp, unsigned int counts)
{
    int index = p_proc_current->task.filp[fd]->dev_index;
    if(index != 5)  
        return 0;       //目前只给ext2做了getdents
    return vfs_table[index].op->getdents(fd, dirp, counts);  //modified by mingxuan 2020-10-18
}